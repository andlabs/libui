...
#define areaGLWidgetType (areaGLWidget_get_type())
#define areaGLWidget(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), areaGLWidgetType, areaGLWidget))
#define isAreaWidget(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), areaGLWidgetType))
#define areaGLWidgetClass(class) (G_TYPE_CHECK_CLASS_CAST((class), areaGLWidgetType, areaGLWidgetClass))
#define isAreaWidgetClass(class) (G_TYPE_CHECK_CLASS_TYPE((class), areaGLWidget))
#define getAreaWidgetClass(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), areaGLWidgetType, areaGLWidgetClass))

typedef struct areaGLWidget areaGLWidget;
typedef struct areaGLWidgetClass areaGLWidgetClass;
...
struct areaGLWidgetClass {
	GtkGLAreaClass parent_class;
};

struct uiArea {
	uiUnixControl c;
	GtkWidget *widget;		// either swidget or areaGLWidget depending on whether it is scrolling

	GtkWidget *swidget;
	GtkContainer *scontainer;
	GtkScrolledWindow *sw;

	GtkWidget *areaGLWidget;
...
G_DEFINE_TYPE(areaGLWidget, areaGLWidget, GTK_TYPE_GL_AREA)

static void areaGLWidget_init(areaGLWidget *aw)
{
	// for events
...
static void areaGLWidget_dispose(GObject *obj)
{
	G_OBJECT_CLASS(areaGLWidget_parent_class)->dispose(obj);
}

static void areaGLWidget_finalize(GObject *obj)
{
	G_OBJECT_CLASS(areaGLWidget_parent_class)->finalize(obj);
}

static void areaGLWidget_size_allocate(GtkWidget *w, GtkAllocation *allocation)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;

	// GtkDrawingArea has a size_allocate() implementation; we need to call it
	// this will call gtk_widget_set_allocation() for us
	GTK_WIDGET_CLASS(areaGLWidget_parent_class)->size_allocate(w, allocation);
...
static void loadAreaSize(uiArea *a, double *width, double *height)
{
	...
	*height = 0;
	...
	if (!a->scrolling) {
		gtk_widget_get_allocation(a->areaGLWidget, &allocation);
...
static gboolean areaGLWidget_draw(GtkGLArea *w, GdkGLContext *cr, gpointer user_data)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
	uiAreaDrawParams dp;

	dp.Context = newContext(NULL);
...
static void areaGLWidget_get_preferred_height(GtkWidget *w, gint *min, gint *nat)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;

	// always chain up just in case
	GTK_WIDGET_CLASS(areaGLWidget_parent_class)->get_preferred_height(w, min, nat);
	if (a->scrolling) {
		*min = a->scrollHeight;
		*nat = a->scrollHeight;
	}
...
static void areaGLWidget_get_preferred_width(GtkWidget *w, gint *min, gint *nat)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;

	// always chain up just in case
	GTK_WIDGET_CLASS(areaGLWidget_parent_class)->get_preferred_width(w, min, nat);
	if (a->scrolling) {
		*min = a->scrollWidth;
		*nat = a->scrollWidth;
	}
...
static gboolean areaGLWidget_button_press_event(GtkWidget *w, GdkEventButton *e)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
...
static gboolean areaGLWidget_button_release_event(GtkWidget *w, GdkEventButton *e)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
	uiAreaMouseEvent me;
...
static gboolean areaGLWidget_motion_notify_event(GtkWidget *w, GdkEventMotion *e)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
	uiAreaMouseEvent me;
...
static gboolean onCrossing(areaGLWidget *aw, int left)
{
	uiArea *a = aw->a;
...
static gboolean areaGLWidget_enter_notify_event(GtkWidget *w, GdkEventCrossing *e)
{
	return onCrossing(areaGLWidget(w), 0);
}

static gboolean areaGLWidget_leave_notify_event(GtkWidget *w, GdkEventCrossing *e)
{
	return onCrossing(areaGLWidget(w), 1);
}
...
static gboolean areaGLWidget_key_press_event(GtkWidget *w, GdkEventKey *e)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
...
static gboolean areaGLWidget_key_release_event(GtkWidget *w, GdkEventKey *e)
{
	areaGLWidget *aw = areaGLWidget(w);
	uiArea *a = aw->a;
...
static void areaGLWidget_set_property(GObject *obj, guint prop, const GValue *value, GParamSpec *pspec)
{
	areaGLWidget *aw = areaGLWidget(obj);
...
static void areaGLWidget_get_property(GObject *obj, guint prop, GValue *value, GParamSpec *pspec)
{
	G_OBJECT_WARN_INVALID_PROPERTY_ID(obj, prop, pspec);
}

static void areaGLWidget_class_init(areaGLWidgetClass *class)
{
	G_OBJECT_CLASS(class)->dispose = areaGLWidget_dispose;
	G_OBJECT_CLASS(class)->finalize = areaGLWidget_finalize;
	G_OBJECT_CLASS(class)->set_property = areaGLWidget_set_property;
	G_OBJECT_CLASS(class)->get_property = areaGLWidget_get_property;

	GTK_WIDGET_CLASS(class)->size_allocate = areaGLWidget_size_allocate;
	//GTK_WIDGET_CLASS(class)->draw = areaGLWidget_draw;
	GTK_WIDGET_CLASS(class)->get_preferred_height = areaGLWidget_get_preferred_height;
	GTK_WIDGET_CLASS(class)->get_preferred_width = areaGLWidget_get_preferred_width;
	GTK_WIDGET_CLASS(class)->button_press_event = areaGLWidget_button_press_event;
	GTK_WIDGET_CLASS(class)->button_release_event = areaGLWidget_button_release_event;
	GTK_WIDGET_CLASS(class)->motion_notify_event = areaGLWidget_motion_notify_event;
	GTK_WIDGET_CLASS(class)->enter_notify_event = areaGLWidget_enter_notify_event;
	GTK_WIDGET_CLASS(class)->leave_notify_event = areaGLWidget_leave_notify_event;
	GTK_WIDGET_CLASS(class)->key_press_event = areaGLWidget_key_press_event;
	GTK_WIDGET_CLASS(class)->key_release_event = areaGLWidget_key_release_event;
...
uiArea *uiNewOpenGLArea(uiAreaHandler *ah)
{
	...
	a->areaGLWidget = GTK_WIDGET(g_object_new(areaGLWidgetType,
		"libui-area", a,
		NULL));
	a->drawingArea = GTK_GL_AREA(a->areaGLWidget);
	a->area = areaGLWidget(a->areaGLWidget);

	a->widget = a->areaGLWidget;

    g_signal_connect (a->widget, "render", G_CALLBACK (areaGLWidget_draw), NULL);
	return a;
}
